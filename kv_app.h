 ...
 */
    ...
    public:
        ...
        //FPT Impl.
        void SetPriorityNum(int num){
            //printf("KVWorker::SetPriorityNum, num: %d\n",num);
            Postoffice::Get()->FPTVan()->SetPriorityNum(num);
        }
        ...
        //FPT Impl.
        Key DecodeKey(Key big_key){


            Key init_key = 0;
            bool change = false;
            std::vector<Range> kr = Postoffice::Get()->GetServerKeyRanges();
            for(int i=0; i< (int)kr.size();i++){
                if(big_key >= kr[i].begin() && big_key < kr[i].end()){
                    init_key =  big_key - kr[i].begin();
                    change = true;
                }

            }
            if(!change){
                printf("KVWorker::DecodeKey, error\n");
                exit(1);
            }
            return init_key;
        }

        //FPT Impl. TODU : better priority initialization
        //  sync one tensor, this tensor may include num_server small tensors,
        //  which means that param keys include num_server ps_keys.
        int FPTPush(const SArray <Key> &keys,
                    const SArray <Val> &vals,
                    const SArray<int> &lens = {},
                    int priority = 0,
                    int cmd = 0,
            ...
            Key init_key = DecodeKey(*(keys.data()));

            Postoffice::Get()->FPTVan()->NewTracker(init_key, vals.size()*sizeof(Val), cb);
            ...
            FPTSend(ts, true, cmd, kvs, priority);
            return ts;
        }
        ...
        //FPT Impl.
        void FPTPull(const SArray <Key> &keys,
                    SArray <Val> *vals,
                    SArray<int> *lens = nullptr,
                    int cmd = 0,
                    const Callback &cb = nullptr) {
            FPTPull_(keys, vals, lens, cmd, cb);
        }
    ...
    private:
        ...
        //FPT Impl.
        template<typename C, typename D>
        void FPTPull_(const SArray <Key> &keys, C *vals, D *lens,
                  int cmd, const Callback &cb);
        ...
        //FPT Impl.
        void FPTSend(int timestamp, bool push, int cmd, const KVPairs<Val> &kvs, int priority);
...
///////////////////////////////////////////////////////////////////////////////
    ...
    //FPT Impl.
    template<typename Val>
    void KVWorker<Val>::FPTSend(int timestamp, bool push, int cmd, const KVPairs<Val> &kvs, int priority) {
        // slice the message
        SlicedKVs sliced;
        slicer_(kvs, Postoffice::Get()->GetServerKeyRanges(), &sliced);
        ...
        if ((size_t) skipped == sliced.size()) {
            printf("KVWorker::FPTSend, error\n");
            exit(1);
            //Postoffice::Get()->FPTVan()->RunCallback(timestamp);
        }
        ...
        for (size_t i = 0; i < sliced.size(); ++i) {
            ...
            Postoffice::Get()->FPTVan()->Send(msg);
        }
    }
    ...
    void KVWorker<Val>::FPTPull_(
        ...
        for (size_t i = 0; i < sliced.size(); ++i) {
            if (!sliced[i].first){
                ...
                const auto &s = sliced[i];
                const auto &kvs = s.second;
                SArray<char> char_vals(kvs.vals);
                Postoffice::Get()->FPTVan()->WriteBack(kvs.keys, char_vals);
            }
        }
