...
# with the License.  You may obtain asg copy of the License at
#
...
context = mx.gpu(bps.local_rank())
nworker = bps.size()
rank = bps.rank()


num_gpus = len([1 for i in opt.gpus.split(',')] )
...
def train(opt, ctx):
    ...
    if opt.optimizer== 'sgd':
         ...
         optimizer_params={'learning_rate': opt.lr, 'wd': opt.wd,   'multi_precision': True}
    else :
         optimizer_params={'learning_rate': opt.lr,  'wd': opt.wd, 'multi_precision': True}
                            
    trainer = bps.DistributedTrainer(net.collect_params(), opt.optimizer, optimizer_params)
        ...
        for i, batch in enumerate(train_data):
            ...
            if opt.log_interval and not (i+1)%opt.log_interval:
                ...
                logger.info('Epoch[%d] Batch [%d]\tSpeed: %f samples/sec\t%s=%f, %s=%f'%(
                               epoch, i, total_samples/(time.time()-btic), name[0], acc[0], name[1], acc[1]))
                total_samples = 0
                btic = time.time()
            ...
            if(opt.iterations > 0 and num_iterations >= opt.iterations):
                print("\ntrain finish\n")
                return
